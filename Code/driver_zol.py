...
from sklearn.metrics import precision_recall_fscore_support,roc_curve,auc,zero_one_loss,f1_score
...
if __name__== '__main__':
    ...
    clf_GradientBooster = xgb.XGBClassifier(max_depth=20, n_estimators=1000, learning_rate=0.1, seed=27).fit(train_features, train_labels)
    ...
    print "\nZero One Losses:"
    print zero_one_losses_list

    indices = []
    for item in (sorted(zero_one_losses_list))[:3]:
        indices.append(zero_one_losses_list.index(item))

    print "\nBase Model Selection (using Zero Loss Error- ZOL):\n"
    base_classifiers = [indices.index((sorted(indices))[0]),indices.index((sorted(indices))[-1])]
