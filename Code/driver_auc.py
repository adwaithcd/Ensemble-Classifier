...
if __name__== '__main__':
    ...
    clf_GradientBooster = xgb.XGBClassifier(max_depth=10, n_estimators=500, learning_rate=0.1, seed=27).fit(train_features, train_labels)
    ...
    print "\n3 Least:"
    print (sorted(auc_list,reverse=True))[:3]
    indices = []
    for item in (sorted(auc_list,reverse=True))[:3]:
        indices.append(auc_list.index(item))

    print "\n Chosen 3 classifiers"
    print(sorted(indices))
    print "\nBase Model Selection (using highest AUC scores):\n"
